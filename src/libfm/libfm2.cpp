...
#include "src/fm_learn_vb_simultaneous_hiar.h"
#include "src/fm_learn_vb_online_simultaneous.h"
#include "src/fm_learn_vb_hiar.h"
...
int main(int argc, char **argv) {
	...
	try {
		...
		// Seed
		long int seed = cmdline.getValue(param_seed, 1);  //use time(NULL) to make random seed
		srand ( seed );
...
//			);
			...
			std::cout << "Loading test data members... \t" << std::endl;
//			DataSubset test(
//				cmdline.getValue(param_cache_size, 0),
//				! (!cmdline.getValue(param_method).compare("mcmc")), // no original data for mcmc
//				! (!cmdline.getValue(param_method).compare("sgd") || !cmdline.getValue(param_method).compare("sgda")) // no transpose data for sgd, sgda
...
//------------------------------------------------------------------------------------------------------
			...
			ifstream train_file(cmdline.getValue(param_train_file).c_str());
			train.num_cases = 0;
			train.num_feature = 0;
			...
			ifstream test_file(cmdline.getValue(param_test_file).c_str());
			test.num_cases = 0;
			test.num_feature = 0;
...
//----------------------------------------------------------------------------------------------------------------------
		...
		} else if (! cmdline.getValue(param_method).compare("vb_hiar")) {
			fm.w.init_normal(fm.init_mean, fm.init_stdev);
	 		fml = new fm_learn_vb_simultaneous_hiar();
			fml->validation = validation;
			((fm_learn_vb_hiar*)fml)->num_iter = cmdline.getValue(param_num_iter, 100);
			((fm_learn_vb_hiar*)fml)->num_eval_cases = cmdline.getValue(param_num_eval_cases, test.num_cases);
		...
		{
			std::cout<<"hi\n";
			fml->learn(train, test, cmdline.getValue(param_train_file), cmdline.getValue(param_test_file));
		}
		else
		{

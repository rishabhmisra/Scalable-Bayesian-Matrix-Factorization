...
int main()
{
	...
	ifstream train("../../data/m1m/train_sbpmf");
	uint num_ratings = 0;
	while (!train.eof())
	{
	...
	train.open("../../data/m1m/train_sbpmf");
	uint train_case = 0;
	while (!train.eof())
	{
		uint movie_id,user_id;
	...
	train.close();
	ifstream test("../../data/m1m/test_sbpmf");
	...
	*/
	...
	for(uint iter = 0; iter<(collection_iter + burn_iter); iter++)
	{
		...
		for(uint k=0; k<D; k++)
		{
			...
			mu_2_star = sigma_2_star*(sigma_2*mu_2 + sigma_u(k)*temp2);
			...
			mu_1_star = sigma_1_star*(sigma_1*mu_1 + sigma_v(k)*temp2);
		...
		//ofstream train("./save/data.txt",std::ofstream::out | std::ofstream::app);
		// calculate RMSE
		//############################################################################################### 
		if (iter>=0)
		{

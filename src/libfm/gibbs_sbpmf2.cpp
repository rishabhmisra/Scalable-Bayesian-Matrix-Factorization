...
int main()
{
	...
	ifstream train("../../data/m100k/train");
	uint num_ratings = 0;
	while (!train.eof())
	{
	...
	for(uint iter = 0; iter<(collection_iter + burn_iter); iter++)
	{
		...
		*/
		...
		for(uint k=0; k<D; k++)
		{
			...
			beta_u_k_star = beta_0 + nu_0 * (mu_u(k)-mu_0) * (mu_u(k)-mu_0) + (0.5) * temp;
			...
			beta_v_k_star = beta_0 + nu_0 * (mu_v(k)-mu_0) * (mu_v(k)-mu_0) + (0.5) * temp;
			...
			mu_v_k_star = sigma_u_k_star*(nu_0 * mu_0 * sigma_v(k) + sigma_v(k)*temp2);

...
class fm_learn_vb_simultaneous : public fm_learn_vb {
	protected:
		...
		virtual void _learn(Data& train, Data& test) {
			...
			DVector<Data*> main_data(num_data);		// to store test and train data
			DVector<e_q_term*> main_cache(num_data);// to store intermediate term for calc. of Rij
			main_data(0) = &train;
			main_data(1) = &test;
			main_cache(0) = cache;
			main_cache(1) = cache_test;
			DVector<Data*> main_data_only_test(num_data_only_test);
			...
			for (uint i = num_complete_iter; i < num_iter; i++) {		// iterations over data
			...
			ofstream initialize("../../data/save_sbpmf");

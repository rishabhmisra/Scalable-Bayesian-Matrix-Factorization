...
/*
			...
			double alpha_old=alpha,alpha_sum=0.0;
			//predict_t_and_write_to_qterms(&train, cache_t);
			for (uint i = 0; i < train.num_cases; i++) {
				alpha_temp += cache[i].e * cache[i].e + cache_t[i].t;
        	                //alpha = alpha_old + new_w0*((_size/(2*alpha)) - (_size/2)*alpha_temp);
				//alpha_sum+=alpha;
			}
			//alpha = alpha_sum/train.num_cases;
			//alpha_old=alpha;
			//alpha_temp/=train.num_cases;
			alpha=alpha_old + new_w0*((1.0/(2*alpha)) - (1.0/2)*alpha_temp);
			...
			//update sigma_0
			sigma_0= sigma_0 + new_w0*((1.0/(2*sigma_0)) - 0.5*(mu_0_dash*mu_0_dash + sigma_0_dash));
			//update sigma_w
			...
			for (uint i=0; i< meta->num_attr_groups; i++) {
				//w_sigma_temp(i)/=(double) meta->num_attr_per_group(i);
				sigma_w(i)=sigma_w(i) + new_w0*( ((double) meta->num_attr_per_group(i)/(2*sigma_w(i))) - 0.5*w_sigma_temp(i));
			}
			...
			for (int f=0;f<fm->num_factor;f++) {
				...
				for (uint g=0; g<meta->num_attr_groups; g++) {
					//v_sigma_temp(g)/=(double) meta->num_attr_per_group(g);
					sigma_v(g,f) = sigma_v(g,f) + new_w0*(((double) meta->num_attr_per_group(g)/(2*sigma_v(g,f))) - 0.5*v_sigma_temp(g));
				}
			}
*/
			//update alpha
		...
		void update_w0(DataSubset& train,uint _size) {
			...
			//mu_0_dash = natural_mu_0_dash/natural_sigma_0_dash;
                        //sigma_0_dash = 1/natural_sigma_0_dash;
			//w0_temp/=train.num_cases;

			//natural_sigma_0_dash = (((1-neww)*sigma_old) + neww*(sigma_0 + _size * alpha)/train.num_cases);
                        //natural_mu_0_dash = (((1-neww)*mu_old) + neww*_size*alpha*w0_temp)/train.num_cases;

                        //t_w0+=train.num_cases;
		...
		void update_w (double& mu, double& sigma, double& sigma_w, sparse_row<DATA_FLOAT>& feature_data,uint _size,uint col,uint bs) {
			...
			for (uint i_fd = 0; i_fd < feature_data.size; i_fd++) {
				...
				//t_wj(col)+=1;
	                        //new_wj(col) = pow((t0_wj+t_wj(col)),-lamda);
				//mu = natural_mu_w_dash(col)/natural_sigma_w_dash(col);
				//sigma = 1/natural_sigma_w_dash(col);
			}
			t_wj(col)+=feature_data.size;
			new_wj(col) = pow(double(t0_wj+t_wj(col)),-lamda);
			//w_mean/=feature_data.size;

			//natural_sigma_w_dash(col) = ((1-neww)*sigma_old) + neww*(sigma_w + alpha*_size*(w_sigma_sqr/feature_data.size))/bs;
                        //natural_mu_w_dash(col) = (((1-neww)*mu_old) + neww* _size * alpha * w_mean)/bs;
		...
		void update_v(int& f, double& mu, double& sigma, double& sigma_v_g, sparse_row<DATA_FLOAT>& feature_data,uint _size,uint col,uint bs) {
			...
			//new_vj(col) = pow(double(t0_vj+t_vj(col)),-lamda);
			for (uint i_fd = 0; i_fd < feature_data.size; i_fd++) {
				...
				//t_vj(col)+=1;
	                        //new_vj(col) = pow((t0_vj+t_vj(col)),-lamda);
				//mu = natural_mu_v_dash(f,col)/natural_sigma_v_dash(f,col);
				//sigma = 1/natural_sigma_v_dash(f,col);
			}
			//new_vj(col) = pow(double(t0_vj+t_vj(col)),-lamda);
			//v_mean/=feature_data.size;
			//natural_sigma_v_dash(f,col) = ((1-neww)*sigma_old + neww*(sigma_v_g + alpha*_size*(v_sigma_sqr/feature_data.size)))/bs;
                        //natural_mu_v_dash(f,col) = (((1-neww)*mu_old) + neww*_size*alpha*v_mean)/bs;

                        //t_vj(col)+=feature_data.size;
...
//--------------------------------------------------------------------------------------------------
			ifstream train_file("/home/avijit/backup/data/train_libfm");
			//ifstream train_file("/home/avijit/rishabh/data/ra.train_libfm");

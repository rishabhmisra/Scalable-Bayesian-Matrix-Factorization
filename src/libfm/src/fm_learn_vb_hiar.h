...
#ifndef FM_LEARN_VB_HIAR_H_
#define FM_LEARN_VB_HIAR_H_
...
class fm_learn_vb_hiar : public fm_learn {
	public:
	...
	public:
		...
		uint num_eval_cases;
		double alpha;
		double alpha_0, beta_0, alpha_1, beta_1, alpha_2, beta_2, mu_0, sigma_0, mu_1, sigma_1, mu_2, sigma_2;

		double alpha_0_dash, beta_0_dash, mu_0_dash, sigma_0_dash, mu_w_bar, sigma_w_bar, alpha_w_dash, beta_w_dash;

		DVectorDoubleVB mu_v_bar, sigma_v_bar;
		DVectorDoubleVB alpha_v_dash, beta_v_dash;
		...
		uint nan_alpha, nan_alpha_0_dash, nan_beta_0_dash, nan_mu_0_dash, nan_sigma_0_dash, nan_mu_w_bar, nan_sigma_w_bar, nan_alpha_w_dash,
		nan_beta_w_dash, nan_mu_v_bar, nan_sigma_v_bar, nan_alpha_v_dash, nan_beta_v_dash, nan_mu_w_dash,
		nan_sigma_w_dash, nan_mu_v_dash, nan_sigma_v_dash;


		uint inf_alpha, inf_alpha_0_dash, inf_beta_0_dash, inf_mu_0_dash, inf_sigma_0_dash, inf_mu_w_bar, inf_sigma_w_bar, inf_alpha_w_dash,
		inf_beta_w_dash, inf_mu_v_bar, inf_sigma_v_bar, inf_alpha_v_dash, inf_beta_v_dash, inf_mu_w_dash,
		inf_sigma_w_dash, inf_mu_v_dash, inf_sigma_v_dash;
...
// calculate T terms and store in q for all the training data points
//
	...
	protected:
		...
		// update w's parameters
			if (fm->k1) {
				...
				for (uint i = 0; i < train.data_t->getNumRows(); i++) {
					...
					update_w(mu_w_dash(row_index), sigma_w_dash(row_index), *feature_data);
				}
			}
		// update v's parameters
			if (fm->num_attribute>0) {
				for (int f = 0; f < fm->num_factor; f++) {
					...
					for (uint i = 0; i < train.data_t->getNumRows(); i++) {
						...
						update_v(f, v[row_index], v1[row_index], *feature_data);
			...
			//alpha_0_dash
			alpha_0_dash=.5 * train.num_cases + alpha_0;
			...
			for (uint i = 0; i < train.num_cases; i++) {
				beta_temp += cache[i].e * cache[i].e + cache_t[i].t;
			}
			beta_old=beta_0_dash;
			alpha=(double) train.num_cases / (beta_temp);
			beta_0_dash=0.5 * beta_temp + beta_0;
			// check for out of bounds values
			if (std::isnan(beta_0_dash)) {
				nan_beta_0_dash++;
				beta_0_dash=beta_old;
				assert(! std::isnan(beta_old));
				assert(! std::isnan(beta_0_dash));
				return;
			}
			if (std::isinf(beta_0_dash)) {
				inf_beta_0_dash++;
				beta_0_dash=beta_old;
				assert(! std::isinf(beta_old));
				assert(! std::isinf(beta_0_dash));
				return;
			}
			}
		// update hyperprior parameters
			if (fm->k1) {
				// alpha_w_dash
				alpha_w_dash=.5 * fm->num_attribute + alpha_1;
				//std::cout<<mu_w_dash.dim<<std::endl;
				// beta_w_dash
				{
	 			double beta_temp=0.0;
				double beta_old=0.0;
				...
				for (uint i = 0; i < fm->num_attribute; i++) {
					beta_temp += mu_w_dash.value[i] * mu_w_dash.value[i] + sigma_w_dash.value[i] - 2 * mu_w_dash.value[i] * mu_w_bar + mu_w_bar * mu_w_bar +
					sigma_w_bar;
				}
				beta_old=beta_w_dash;
				beta_w_dash=0.5 * beta_temp + beta_1;

				// check for out of bounds values
				if (std::isnan(beta_w_dash)) {
					nan_beta_w_dash++;
					beta_w_dash=beta_old;
					assert(! std::isnan(beta_old));
					assert(! std::isnan(beta_w_dash));
					return;
				}
				if (std::isinf(beta_w_dash)) {
					inf_beta_w_dash++;
					beta_w_dash=beta_old;
					assert(! std::isinf(beta_old));
					assert(! std::isinf(beta_w_dash));
					return;
				}
				}
				...
				sigma_old=sigma_w_bar;
				sigma_w_bar = 1.0/(mu_w_dash.dim*(alpha_w_dash/beta_w_dash) + sigma_1);

				// check for out of bounds values
				if (std::isnan(sigma_w_bar)) {
					nan_sigma_w_bar++;
					sigma_w_bar=sigma_old;
					assert(! std::isnan(sigma_old));
					assert(! std::isnan(sigma_w_bar));
					return;
				}
				if (std::isinf(sigma_w_bar)) {
					inf_sigma_w_bar++;
					sigma_w_bar=sigma_old;
					assert(! std::isinf(sigma_old));
					assert(! std::isinf(sigma_w_bar));
					return;
				}
				}
				...
				for (uint i=0; i< fm->num_attribute;i++) {
					mu_temp+=mu_w_dash.value[i];
				}

				mu_old=mu_w_bar;
				mu_w_bar = sigma_w_bar * (mu_temp * (alpha_w_dash/beta_w_dash) + sigma_1 * mu_1);

				// check for out of bounds values
				if (std::isnan(mu_w_bar)) {
					nan_mu_w_bar++;
					mu_w_bar=mu_old;
					assert(! std::isnan(mu_old));
					assert(! std::isnan(mu_w_bar));
					return;
				}
				if (std::isinf(mu_w_bar)) {
					inf_mu_w_bar++;
					mu_w_bar=mu_old;
					assert(! std::isinf(mu_old));
					assert(! std::isinf(mu_w_bar));
					return;
				}
				}
			}
			// alpha_v_dash, beta_v_dash, sigma_v_bar, mu_v_bar
			for (int f = 0; f < fm->num_factor; f++) {

				//update alpha_v_dash
				alpha_v_dash.value[f]=.5 * fm->num_attribute + alpha_2;
				...
				double* v1=sigma_v_dash.value[f];
				for (uint i = 0; i < fm->num_attribute; i++) {
					beta_temp += v[i] * v[i] + v1[i] - 2 * v[i] * mu_v_bar.value[f] + mu_v_bar.value[f] *
					mu_v_bar.value[f] + sigma_v_bar.value[f];
				}
				beta_old=beta_v_dash.value[f];
				beta_v_dash.value[f]=0.5 * beta_temp + beta_2;

				// check for out of bounds values
				if (std::isnan(beta_v_dash.value[f])) {
					nan_beta_v_dash++;
					beta_v_dash.value[f]=beta_old;
					assert(! std::isnan(beta_old));
					assert(! std::isnan(beta_v_dash.value[f]));
					return;
				}
				if (std::isinf(beta_v_dash.value[f])) {
					inf_beta_v_dash++;
					beta_v_dash.value[f]=beta_old;
					assert(! std::isinf(beta_old));
					assert(! std::isinf(beta_v_dash.value[f]));
					return;
				}
				}
				...
				sigma_old=sigma_v_bar.value[f];
				sigma_v_bar.value[f] = 1.0/(fm->num_attribute *(alpha_v_dash.value[f]/beta_v_dash.value[f]) + sigma_2);

				// check for out of bounds values
				if (std::isnan(sigma_v_bar.value[f])) {
					nan_sigma_v_bar++;
					sigma_v_bar.value[f]=sigma_old;
					assert(! std::isnan(sigma_old));
					assert(! std::isnan(sigma_v_bar.value[f]));
					return;
				}
				if (std::isinf(sigma_v_bar.value[f])) {
					inf_sigma_v_bar++;
					sigma_v_bar.value[f]=sigma_old;
					assert(! std::isinf(sigma_old));
					assert(! std::isinf(sigma_v_bar.value[f]));
					return;
				}
				}
				...
				mu_old=mu_v_bar.value[f];
				mu_v_bar.value[f] = sigma_v_bar.value[f] * (mu_temp * (alpha_v_dash.value[f]/beta_v_dash.value[f]) +
				sigma_2 * mu_2);

				// check for out of bounds values
				if (std::isnan(sigma_v_bar.value[f])) {
					nan_mu_v_bar++;
					mu_v_bar.value[f]=mu_old;
					assert(! std::isnan(mu_old));
					assert(! std::isnan(mu_v_bar.value[f]));
					return;
				}
				if (std::isinf(mu_v_bar.value[f])) {
					inf_mu_v_bar++;
					mu_v_bar.value[f]=mu_old;
					assert(! std::isinf(mu_old));
					assert(! std::isinf(mu_v_bar.value[f]));
					return;
				}
				}
		...
		void update_w0(DataSubset& train) {
			...
			mu_old=mu_0_dash;
			for (uint i=0; i<train.num_cases;i++) {
				w0_temp+=cache[i].e + mu_0;
			}
			mu_0_dash=sigma_0_dash * (sigma_0 * mu_0 + alpha * w0_temp);
		...
		void update_w (double& mu, double& sigma, sparse_row<DATA_FLOAT>& feature_data) {
			...
			sigma = (double) 1.0 / (alpha_w_dash/beta_w_dash + alpha * w_sigma_sqr);
			mu =  sigma * ((alpha_w_dash/beta_w_dash) * mu_w_bar + alpha * w_mean);
		...
		void update_v(int& f, double& mu, double& sigma, sparse_row<DATA_FLOAT>& feature_data) {
			...
			for (uint i_fd = 0; i_fd < feature_data.size; i_fd++) {
				...
				v_mean += x_li * h * (cache_li->e + h * x_li * mu);
				v_sigma_sqr += x_li * x_li * h * h + x_li * x_li * h1;
			}
			sigma = (double) 1.0 / ((alpha_v_dash.value[f]/beta_v_dash.value[f]) + alpha * v_sigma_sqr);
			mu =  sigma * (mu_v_bar.value[f] * (alpha_v_dash.value[f]/beta_v_dash.value[f]) + alpha * v_mean);
		...
		void free_energy(DataSubset& train) {
			...
			file="free_energy_" + str + "_vb_hiar";
			...
			double temp1 = - .5 * std::log (2 * 3.14) + .5*(std::log (alpha));
			free_energy += temp1 - .5 * alpha * temp;

			//free_energy += alpha_0 * std::log (beta_0) - alpha_0_dash * std::log (beta_0_dash) - loggamma(alpha_0) +
			//loggamma(alpha_0_dash) + (alpha_0 - alpha_0_dash) * (std::log (beta_0_dash) + digamma(alpha_0_dash)) +
			//(alpha_0_dash / beta_0_dash) * (beta_0_dash - beta_0);

			free_energy += - 0.5 * sigma_0 * (mu_0_dash * mu_0_dash + sigma_0_dash - 2 * mu_0_dash * mu_0 + mu_0 * mu_0) +
			0.5 * std::log(sigma_0_dash * sigma_0) +.5;

			for (uint i=0;i<fm->num_attribute;i++) {
				//uint g=meta->attr_group(i);
				free_energy += - 0.5 * (alpha_w_dash / beta_w_dash) * (mu_w_dash.value[i] * mu_w_dash.value[i] +
				sigma_w_dash.value[i] - 2 * mu_w_dash.value[i] * mu_w_bar + mu_w_bar * mu_w_bar + sigma_w_bar) +
				0.5 * (std::log(beta_w_dash) + digamma(alpha_w_dash)) + .5 * std::log (sigma_w_dash.value[i]) + .5;
			}
			for (int f=0;f<fm->num_factor;f++) {
				double* v=mu_v_dash.value[f];
				double* v1=sigma_v_dash.value[f];
				for (uint i=0;i<fm->num_attribute;i++) {
					//uint g=meta->attr_group(i);
				free_energy += - 0.5 * (alpha_v_dash.value[f] / beta_v_dash.value[f]) * (v[i] * v[i] + v1[i] -
				2 * v[i] * mu_v_bar.value[f] + mu_v_bar.value[f] * mu_v_bar.value[f] + sigma_v_bar.value[f]) +
				0.5 * (std::log(beta_v_dash.value[f]) + digamma(alpha_v_dash.value[f])) + .5 * std::log (v1[i]) + .5;
				}
			}

			free_energy += .5 * std::log (sigma_1 * sigma_w_bar) - .5 * sigma_1 *
			(mu_w_bar * mu_w_bar + sigma_w_bar- 2 * mu_w_bar * mu_1 + mu_1 * mu_1) + .5;

			for (int f=0; f<fm->num_factor ;f++) {
				free_energy += .5 * std::log (sigma_2 * sigma_v_bar.value[f]) - .5 * sigma_2 * (mu_v_bar.value[f] *
				mu_v_bar.value[f] + sigma_v_bar.value[f]- 2 * mu_v_bar.value[f] * mu_2 + mu_2 * mu_2) + .5;
			}

			free_energy += alpha_1 * std::log (beta_1) - alpha_w_dash * std::log (beta_w_dash) - loggamma(alpha_1) +
			loggamma(alpha_w_dash) + (alpha_1 - alpha_w_dash) * (std::log (beta_w_dash) + digamma(alpha_w_dash)) +
			(alpha_w_dash / beta_w_dash) * (beta_w_dash - beta_1);

			for (int f=0; f<fm->num_factor ;f++) {
				free_energy += alpha_1 * std::log (beta_1) - alpha_v_dash.value[f] * std::log (beta_v_dash.value[f]) -
				loggamma(alpha_1) + loggamma(alpha_v_dash.value[f]) + (alpha_1 - alpha_v_dash.value[f]) *
				(std::log (beta_v_dash.value[f]) + digamma(alpha_v_dash.value[f])) +
				(alpha_v_dash.value[f] / beta_v_dash.value[f]) * (beta_v_dash.value[f] - beta_1);
		...
		double loggamma(double z) {
			return z * std::log (z) - z - .5 * std::log (z / (2 * 3.14)) + 1.0 / (12 * z) -
			1.0 / (360 * z * z * z) + 1.0 / (1260 * z * z * z * z * z);
		}

		double digamma(double z) {
			return std::log (z) -1.0/(2 * z)-1.0/(12 * z * z) + 1.0/(120 * z * z * z * z) - 1.0 / (256 * z * z * z * z * z * z);
		}
	public:
		virtual void init() {
			fm_learn::init();
			...
			// set dimension
			mu_v_bar.setSize(fm->num_factor_new);
			sigma_v_bar.setSize(fm->num_factor_new);
			alpha_v_dash.setSize(fm->num_factor_new);
			beta_v_dash.setSize(fm->num_factor_new);
			...
			mu_v_bar.init_normal(0,1);
			sigma_v_bar.init(.02);
			alpha_v_dash.init_gamma(1,1);
			beta_v_dash.init(1);

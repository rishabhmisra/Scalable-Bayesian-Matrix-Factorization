...
class fm_learn_vb_simultaneous : public fm_learn_vb {
	protected:
		...
		virtual void predict(DataSubset&, DVector<double>&) {}
		...
		virtual void _learn(Data& train, Data& test) {
			...
			for (uint i = num_complete_iter; i < num_iter; i++) {
				...
				if (task == TASK_REGRESSION) {
					...
					double acc_test_this, mapatk;
					uint k=5;
					//_evaluate_class(pred_this, test.target, 1.0, acc_test_this, ll_test_this, num_eval_cases);
					_evaluate_class_map(pred_this, test.target, 1.0, acc_test_this, mapatk, k, num_eval_cases);
					
					std::cout << "#Iter=" << std::setw(3) << i << "\tTest=" << acc_test_this <<" MAP@"<<k<<"="<<mapatk << std::endl;

		...
		// update w's parameters
			...
			if (fm->k1) {
				...
				for (uint i = 0; i < train.data_t->getNumRows(); i++) {
					...
					update_w(mu_w_dash(row_index), sigma_w_dash(row_index), sigma_w(g), *feature_data,train.num_cases,i);
				}
			}
...
*/
		...
		void update_w (double& mu, double& sigma, double& sigma_w, sparse_row<DATA_FLOAT>& feature_data,uint _size,uint col) {
			...
			for (uint i_fd = 0; i_fd < feature_data.size; i_fd++) {
				...
				natural_sigma_w_dash(col) = ((1-new_wj(col))*sigma_old) + new_wj(col)* (sigma_w + alpha * w_sigma_sqr);
				natural_mu_w_dash(col) = ((1-new_wj(col))*mu_old) + new_wj(col) * _size * alpha * w_mean;
		...
		void update_v(int& f, double& mu, double& sigma, double& sigma_v_g, sparse_row<DATA_FLOAT>& feature_data,uint _size,uint col) {
			...
			for (uint i_fd = 0; i_fd < feature_data.size; i_fd++) {
				...
				natural_sigma_v_dash(f,col) = (1-new_vj(col))*sigma_old + new_vj(col)*(sigma_v_g + alpha * v_sigma_sqr);
				natural_mu_v_dash(f,col) = ((1-new_vj(col))*mu_old) + new_vj(col) * _size*alpha * v_mean;

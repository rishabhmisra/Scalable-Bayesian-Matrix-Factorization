...
class fm_learn_vb : public fm_learn {
	...
	protected:
		...
		*/
		void predict_data_and_write_to_eterms(DVector<Data*>& main_data, DVector<e_q_term*>& main_cache) {	// for Rij of equation 27 and store it in e
			...
			for (int f = 0; f < fm->num_factor; f++) {		// for f 1 to k
				...
				for (uint ds = 0; ds < main_cache.dim; ds++) {
					...
					Data* m_data = main_data(ds);			// store part of data
...
//
		void predict_t_and_write_to_qterms(Data* main_data, t_term* main_cache) {

 			t_term* m_cache=main_cache;
			Data* m_data = main_data;
			// do this using only the transpose copy of the training data:
	...
	protected:
		...
		void update_all(Data& train) {		// k0 and k1 are bias terms.. if k0 is 1 then only include w0
		...
		// update w's parameters
			...
			if (fm->k1) {			// if k1 is 1 then only include Wj's
				...
				for (uint i = 0; i < train.data_t->getNumRows(); i++) {
					...
					uint g = meta->attr_group(row_index);			// retrieve the attribute group
					update_w(mu_w_dash(row_index), sigma_w_dash(row_index), sigma_w(g), *feature_data);	// this is to be parallelised	// call function to update all the w's
				}
			}
		...
		// update v's parameters
			if (fm->num_attribute>0) {
				for (int f = 0; f < fm->num_factor; f++) {
					for (uint c = 0; c < train.num_cases; c++) {		// initialise with zero
						cache[c].q = 0.0;
						cache_t[c].q = 0.0;
						cache_t[c].z = 0.0;
					}
	...
	public:
		...
		virtual void init() {
			...
			/*
			int state = 1;
			ifstream initialize("../../data/save_sbpmf");
    		while (!initialize.eof()) {

            		double read;
					std::string line;
					if(state == 1)
					{
						std::getline(initialize, line);
						if (sscanf(line.c_str(), "%lf", &read) >=1) {
							alpha = read;
						}
						state = 2;
					}
					...
					{
						for(uint i=0; i<fm->num_factor; i++)
						{
